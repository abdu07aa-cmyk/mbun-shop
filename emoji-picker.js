...
async function seedProductsIfEmpty() {
  ...
  Utils.showToast('Memuat produk bawaan...', 'info');

  // Simpan semua ke localStorage sekaligus (cepat, tanpa API calls)
  const withIds = SEED_PRODUCTS.map(p => ({
    ...p,
    id: Utils.generateId('PRD'),
    created_at: new Date().toISOString(),
  }));

  Offline.saveLocal(CONFIG.TABLES.PRODUCTS, withIds);
  STATE.setProducts(withIds);

  // Coba sync ke Supabase di background (tidak blocking)
  if (API.isConfigured()) {
    API.insert(CONFIG.TABLES.PRODUCTS, withIds)
      .catch(() => {}); // diam-diam jika gagal
  }

  Utils.showToast(`${withIds.length} produk bawaan berhasil dimuat ✅`, 'success');
}
